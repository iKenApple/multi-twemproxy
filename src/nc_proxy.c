...
struct conn *p; //connections for event add
...
rstatus_t
proxy_each_addEvent(void *elem, void *data)
{
    rstatus_t status;
    struct server_pool *pool = elem;
...
rstatus_t
proxy_addEvent(struct context *ctx)
{
    rstatus_t status;

    ASSERT(array_n(&ctx->pool) != 0);

    status = array_each(&ctx->pool, proxy_each_addEvent, NULL);
    if (status != NC_OK) {
        proxy_deinit(ctx);
        return status;
    }
...
proxy_accept(struct context *ctx, struct conn *p)
{
    ...
    for (;;) {
        ...
        //printf("accept %s pid: %d sd: %d\n", strerror(errno), getpid(), sd);
        if (sd < 0) {
            if (errno == EINTR) {

...
static int 
core_master_loop(struct instance *nci, struct context *ctx) 
{
    int i;
    pid_t pid;
    rstatus_t status;
        ...
        while (1) {
            ...
            if (pid > 0) {
                ...
                if (new_pid == 0) {
                    ...
                    for (i = 0; i < core_worker_num; i++) {
                        if (core_worker_list[i] == pid) {
                            core_worker_list[i] = new_pid;
                            break;
                        }
                    }
                    continue;
...
core_core(void *arg, uint32_t events)
{
    ...
    if (events & EVENT_READ) {
        //printf("handle read event pid: %d\n", getpid());
    ...
    if (events & EVENT_WRITE) {
        //printf("handle write event pid: %d\n", getpid());
